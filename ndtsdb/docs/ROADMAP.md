...
| ~~**P1**~~ | ~~**真实数据验证**~~ | ~~Gorilla 压缩率实测 73%（远超预期 12-30%）~~ | ~~仿真真实数据测试~~ | ~~2 小时~~ | ✅ **已完成 v0.9.4.4** |
| ~~**P2**~~ | ~~**通用压缩评估**~~ | ~~测试 Brotli 压缩率 78.8%（比 Gorilla 更差）~~ | ~~保持 Gorilla 作为默认~~ | ~~3 小时~~ | ✅ **已完成 v0.9.5.0** |
| ~~**P2**~~ | ~~**全表聚合性能优化**~~ | ~~100K 行聚合实测 9.1 ops/sec（远超预期 2 ops/sec）~~ | ~~性能已达标，暂缓优化~~ | ~~2 小时~~ | ✅ **已评估（性能达标）** |
| **P3** | **压缩算法 C 实现** | 压缩/解压性能瓶颈（若出现） | 将 Delta/RLE/Gorilla/Brotli 下放到 libndts（C + SIMD 加速） | 2-3 天 | 💤 **按需触发** |
...
**触发条件**：
- 当压缩/解压成为性能瓶颈时（通过 profiling 确认）
- 写入性能 < 1M rows/s 且瓶颈在压缩
- 查询性能 < 10M rows/s 且瓶颈在解压

**优化方案**：
- 将 Delta/RLE/Gorilla/Brotli 等算法下放到 **libndts**（C + SIMD）
- 利用 FFI 调用 C 实现（零拷贝，直接操作 TypedArray buffer）
- 使用 SIMD 指令集加速（SSE/AVX/NEON）

**预期提升**：
- 压缩速度：10-50x（C + SIMD vs 纯 JS）
- 解压速度：20-100x
- 整体写入吞吐：2-5x

**实施步骤**：
1. 在 `native/ndts.c` 中实现核心压缩算法
2. 导出 FFI 接口（`compress_delta_int64`, `decompress_gorilla_f64` 等）
3. 在 `compression.ts` 中优先调用 C 实现，自动回退到 JS
4. 性能测试验证（吞吐/延迟/CPU 占用）

**参考实现**：
- Gorilla: Facebook 开源 C++ 实现
- Delta/RLE: 简单算法，易于移植
- Brotli: Google 官方 C 实现

**优先级**：💤 **按需触发**（当前 JS 实现性能已足够）
...
| **v0.9.4.4** | 02-11 12:25 | **P1/P3 性能验证**（真实数据测试；聚合性能达标；发现 Gorilla 压缩率问题 73%） |
| **v0.9.5.0** | 02-11 12:35 | **P2 通用压缩评估**（Brotli 集成；对比测试；结论：Gorilla 优于通用压缩） |

...
| ~~getMax 全表扫描~~ | ~~查询最新 timestamp 需扫描所有分区~~ | ✅ PartitionedTable.getMax() + partitionHint（哈希分区直接定位 bucket） | ✅ **已解决 v0.9.3.11** |
...
|--------|------|------|----------|----------|------|
| ~~**P0**~~ | ~~**getMax 全表扫描优化**~~ | ~~查询最新 timestamp 需扫描 100 个分区~~ | ~~PartitionedTable.getMax() + partitionHint~~ | ~~1 小时~~ | ✅ **已完成 v0.9.3.11** |
| **P1** | **全表聚合性能优化** | 100K 行聚合 ~477ms（2 ops/sec） | 1. C FFI 加速聚合计算<br>2. 预计算统计信息缓存<br>3. LIMIT 提前终止 | 2-4 小时 | 🟡 **计划中** |
| **P2** | **真实数据验证** | 目前只有合成数据测试 | 1. Binance 真实 K 线验证<br>2. TV 数据回放测试<br>3. 长期稳定性测试 | 1-2 天 | 🟡 **计划中** |
| **P3** | **zstd 压缩集成** | Gorilla 压缩率仅 12-30% | 1. 集成 zstd（目标 50-70%）<br>2. 算法自动选择策略<br>3. 向后兼容 | 1 天 | 🟡 **计划中** |

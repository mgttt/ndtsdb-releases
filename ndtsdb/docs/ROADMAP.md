...
| 优先级 | 任务 | 问题 | 优化方向 | 预估工期 |
|--------|------|------|----------|----------|
| **P1** | **全表聚合性能优化** | 100K 行聚合 ~477ms（2 ops/sec） | 1. C FFI 加速聚合计算<br>2. 预计算统计信息缓存<br>3. LIMIT 提前终止 | 2-4 小时 |
| **P2** | **真实数据验证** | 目前只有合成数据测试 | 1. Binance 真实 K 线验证<br>2. TV 数据回放测试<br>3. 长期稳定性测试 | 1-2 天 |
| **P3** | **zstd 压缩集成** | Gorilla 压缩率仅 12-30% | 1. 集成 zstd（目标 50-70%）<br>2. 算法自动选择策略<br>3. 向后兼容 | 1 天 |
| **P4** | **错误处理规范化** | 部分 throw，部分 console.warn | 统一返回 Result 类型 | 半天 |
| **P5** | **WAL 事务支持** | 无 ACID，靠 CRC + 原子重写 | 1. Write-Ahead Log<br>2. 崩溃恢复机制 | 2-3 天 |
...
- [ ] **P0: AppendWriter.readHeader() 静态方法缺失** 🔴
  ...
  - **文件**: 
    - 调用方: `ndtsdb/src/partition.ts:73` (loadPartitions)
    - 缺失位置: `ndtsdb/src/append.ts` (需添加静态方法)
  - **优先级**: P0（阻塞生产数据读取）
  - **预估工期**: 5-10 分钟（实现 readHeader 静态方法 + 返回 header JSON）
  - **测试数据**: `/home/devali/moltbaby/quant-lib/data/ndtsdb/` (30 symbols × 100 partitions)

**预计完成**: 
- 核心功能：✅ 已完成（2026-02-10）
- P0 修复：2026-02-10 晚（bot-001）
